...
export const CREDENTIALS_KEY = 'calendar_credentials';

export const parseTokenClaims = (token: string): AuthCredentialClaims =>
  JSON.parse(getAtob()(token.split('.')[1]));

export const checkTokenExpired = (claims: AuthCredentialClaims): boolean =>
...
export const saveTokenToCookie = (credentials: AuthCredentials, remember: boolean) => {
  setCookie(CREDENTIALS_KEY, JSON.stringify(credentials), remember ? 7 : 0);
};
